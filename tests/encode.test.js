describe('encode function tests', () => {
    ...
    test('it can encode buffers', () => {
        expect(encode(Buffer.from('hello bearer token')))
            .toBe('aGVsbG8gYmVhcmVyIHRva2Vu');
        expect(encode(Buffer.from('The quick brown fox jumps over the lazy dog')))
            .toBe('VGhlIHF1aWNrIGJyb3duIGZveCBqdW1wcyBvdmVyIHRoZSBsYXp5IGRvZw==');
        expect(encode(Buffer.from('The early bird catches the worm')))
            .toBe('VGhlIGVhcmx5IGJpcmQgY2F0Y2hlcyB0aGUgd29ybQ==');
        expect(encode(Buffer.from('The pen is mightier than the sword')))
            .toBe('VGhlIHBlbiBpcyBtaWdodGllciB0aGFuIHRoZSBzd29yZA==');
        expect(encode(Buffer.from('All\'s fair in love and war')))
            .toBe('QWxsJ3MgZmFpciBpbiBsb3ZlIGFuZCB3YXI=');
        expect(encode(Buffer.from('Where there\'s smoke, there\'s fire')))
            .toBe('V2hlcmUgdGhlcmUncyBzbW9rZSwgdGhlcmUncyBmaXJl');        
    });
});